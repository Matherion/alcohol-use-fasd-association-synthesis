...
This publication presents figures which are part of 2 larger cohorts. 
...
res$sample$recruitmentSetting <- "community clinic"; ## Is "records", "school", "population", "hospital", or NA
...
res$subsample[["Case mothers pFAS"]] <- list(    ### Create object for this subsample
 ...
 res$subsample[["Control mothers exposed"]] <- list(    ### Create object for this subsample
  comment = "note that these children are heavily exposed to alcohol",
   size = 75                              ### Additional variables specified as univariate data for this subsample
);

 res$subsample[["Control mothers unexposed"]] <- list(    ### Create object for this subsample
  comment = "healthy controls are offspring of abstaining or low-consumption mothers",
  size = 69                              ### Additional variables specified as univariate data for this subsample
);
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "Alcohol consumption: during a day",       ### Name of variable this result pertains to
  subsample = "Case mothers FAS",                   ### Name of relevant subsample, or NA
  mean = 1.8,                        ### Mean or NA
  sd = 2.4,                          ### Standard deviation, or NA
  comment = "extracted from table 3, page e782"                      ### Comment or NA
);
...
res$univariate[[length(res$univariate) + 1]] <- list(
  ...
  mean = 1.3,                        ### Mean or NA
  sd = 1.8,                          ### Standard deviation, or NA
  comment = "extracted from table 3, page e782"                      ### Comment or NA
);
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "Alcohol consumption: during an occasion",       ### Name of variable this result pertains to
  subsample = "Case mothers FAS",                   ### Name of relevant subsample, or NA
  mean = 4.7,                        ### Mean or NA
...
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "Frequency Alcohol consumption: during",       ### Name of variable this result pertains to
  subsample = "Case mothers FAS",                   ### Name of relevant subsample, or NA
  mean = 2.0,                        ### Mean or NA
...
res$univariate[[length(res$univariate) + 1]] <- list(
  ...
  subsample = "Case mothers FAS",                   ### Name of relevant subsample, or NA
  mean = 6.3,                        ### Mean or NA
  sd = 6.2,                          ### Standard deviation, or NA
  comment = "extracted from table 3, page e782"                      ### Comment or NA
);
res$univariate[[length(res$univariate) + 1]] <- list(
  variable = "Smoking during",       ### Name of variable this result pertains to
...
res$univariate[[length(res$univariate) + 1]] <- list(
  ...
  subsample = "Control mothers unexposed",                   ### Name of relevant subsample, or NA
  mean = 4.3,                        ### Mean or NA
  sd = 7.4,                          ### Standard deviation, or NA
  comment = "extracted from table 3, page e782"                      ### Comment or NA
);
...
res$variable[[length(res$variable) + 1]] <- list(
 ...
 ####################################### ONLY FOR FAS DIAGNOSIS VARIABLES
  diagnosisMethod = "IOM05, 4Digit",             ### "IOM96", "IOM05", or "4digit", etc
...
res$variable[[length(res$variable) + 1]] <- list(
 ...
 ####################################### ONLY FOR FAS DIAGNOSIS VARIABLES
  ...
  res$variable[[length(res$variable) + 1]] <- list(
  variable = "Alcohol consumption: during a day",     ### Replace 'example' with variable name
...
####################################### ONLY FOR MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  specificationUnits = "oz",						 ### 'g' als het gemiddelde in grammen is; ‘units’ als het gemiddelde (en sd, etc) in aantal glazen is uitgedrukt
  unitUnits = "oz", 								 ### ‘g’, ‘ml’, of ‘oz’, en specificeert waarin de auteurs een unit (glas) definiëren
  nrOfUnitsInUnit = "0.5",							 ### aantal gram (of milligram of ons, etc) dat volgens de auteurs een unit (standaard glas) is
  calculatedunitUnits = "g",
  calculatednrOfUnitsInUnit = 14.2,				 ### .5 oz = 14.1748 g
  ...
  description = "1 oz AA = 2 standard drinks.",                 ### Description, as text; or NA
  comment = "Table 3, p. e782. Alcohol use during pregnancy oz AA/day"                      ### Comment, as text; or NA
);
 
res$variable[[length(res$variable) + 1]] <- list(
  variable = "Alcohol consumption: during an occasion",     ### Replace 'example' with variable name
  recodeToCategory = "Alcohol consumption during",
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
...
####################################### ONLY FOR MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  period = "during",               		### "before", "1st", "2nd", "3rd", "during", "after", "other"
  varType = "units",               		 ### "frequency" or "units"
  intensitySpecification = "per occasion",                   ### NA, "any day", "weekday", "weekendday", "friday", "saturday",  etc
  #specificationUnits = NA,           			 ### "ml", "mg", "oz"
  specificationUnits = "oz",						 ### 'g' als het gemiddelde in grammen is; ‘units’ als het gemiddelde (en sd, etc) in aantal glazen is uitgedrukt
  unitUnits = "oz", 								 ### ‘g’, ‘ml’, of ‘oz’, en specificeert waarin de auteurs een unit (glas) definiëren
  nrOfUnitsInUnit = "0.5",							 ### aantal gram (of milligram of ons, etc) dat volgens de auteurs een unit (standaard glas) is
  calculatedunitUnits = "g",
  calculatednrOfUnitsInUnit = 14.2,				 ### .5 oz = 14.1748 g, 1 oz = 28.3495
  specificationTimeframe = "per occasion",  				 ### "per week", "per month", "per year"
  aboutBinging = FALSE,             			 ### TRUE if the variable is about binge drinking
  aboutAlcoholism = FALSE,				### TRUE if the variable is about alcoholism
  description = "1 oz AA = 2 standard drinks.",                 ### Description, as text; or NA
  comment = "Table 3, p. e782. Alcohol use during pregnancy oz AA/occasion"                      ### Comment, as text; or NA
);
 
  res$variable[[length(res$variable) + 1]] <- list(
  variable = "Frequency Alcohol consumption: during",     ### Replace 'example' with variable name
  recodeToCategory = "Alcohol consumption during",
  moment = 1,                       ### moment this variable was measured/manipulated
  type = "question",                ### "question", "aggregate", or "manipulation"
  datatype = "numeric",             ### "numeric", "logical", "nominal", "ordinal", or "text"
...
####################################### ONLY FOR MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  comment = "Table 3, p. e782. Alcohol use during pregnancy frequency (days / week)"                      ### Comment, as text; or NA
);
 ...
 ####################################### ONLY FOR MATERNAL DRINKING BEHAVIOR VARIABLES
  ...
  aboutAlcoholism = FALSE,				### TRUE if the variable is about alcoholism
  description = NA,                 ### Description, as text; or NA
  comment = "Table 3, p. e782. Cigarettes/day during pregnancy"                      ### Comment, as text; or NA
);
